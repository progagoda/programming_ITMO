...
public class GeneralColl {
    ...
    public void printInfoAboutCollection() {
        Messages.normalMessageOutput("Тип коллекции - " + getCollection().getClass().getName() + "\n" +
                ...
                "Кол-во элементов - " + getCollection().size(), MessageColor.ANSI_RED);
    ...
    public boolean add_if_min(Scanner scanner) {
        ...
        if (getCollection().size() > 0) {
            ...
            if (group != null) {
                try {
                    while (true) {
                        if (getCollection().size() > 0 && getCollection().peek().compareTo(group)>0) {
                            ...
                            getCollection().add(group);
                            Messages.normalMessageOutput("Элемент был добавлен в коллекцию", MessageColor.ANSI_RED);
                            return  true;
                        } else {
                ...
                Messages.normalMessageOutput("Не удалось получить элемент для сравнения.", MessageColor.ANSI_RED);
                return false;
            }
        } else {
            Messages.normalMessageOutput("В коллекции нет элементов, нечего удалять", MessageColor.ANSI_RED);
            return false;
        }
    ...
    public boolean removeGreatest(Scanner scanner) {
        ...
        Messages.normalMessageOutput("Все элементы, больше данного - удалены!", MessageColor.ANSI_RED);
        return true;
    }
    ...
    public boolean printElementbySemester() {
        if (getCollection().size() > 0) {
            ...
            Messages.normalMessageOutput("Ваша коллекция пуста, выводить нечего", MessageColor.ANSI_RED);
            return  false;
        }
        return  true;
    }
    ...
    public boolean printElementbyGroupAdmin(String groupAdmin) {
        ...
        if (getCollection().size()>0 && groupAdmin!=null) {
            ...
            Messages.normalMessageOutput("Ваша коллеция либо пуста либо вы не ввели некорретное имя искомого админа", MessageColor.ANSI_RED);
        }
        return  true;
        }

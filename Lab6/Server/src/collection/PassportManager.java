...
public class PassportManager {
         ...
         */
        private static final HashSet<Integer> hashSetOfIds = new HashSet<>();
         ...
         */
        public static boolean checkUniq(Integer id) {
     ...
     * Метод - добавление id
     *
     * @param id номер для добавления
     * @return true/false, уникальный или нет
     */
    public static boolean addId(Integer id) {
        return hashSetOfIds.add(id);
    }
        /**
         ...
         */
        public static Integer findUniq(Integer id) {
            if (checkUniq(id)) {
                hashSetOfIds.add(id);
                return id;
            } else {
                return findUniq(Math.abs(new Random().nextInt()));
            }
        }
